namespace OverdriveServer{
    ...
    public static class NetStructures{
        ...
        public class CarLocationData { //Sent by OverdriveServer Tracking (Should be the most accurate)
            ...
            public int trustLevel {get; set;} //how much to trust the data (0 = not trusted, 1 = likely accurate, 2 = certain)
        }
        [System.Serializable]
        ...
        public const string EVENT_UTILITY_LOG = "utility_log"; //Utility log string
        public const string EVENT_CAR_LOCATION = "car_location"; //Car location data (see LocationData)
        public const string EVENT_CAR_TRANSITION = "car_transition"; //Car transition data (see TransitionData)
        public const string EVENT_CAR_SEGMENT = "car_segment"; //Car segment data (see NetSegment)
        public const string EVENT_CAR_DELOCALIZED = "car_delocalized"; //Car delocalized (currently not used, see MSG_CAR_DELOCALIZED for the time being)
        public const string EVENT_CAR_TRACKING_UPDATE = "car_tracking_update"; //Car tracking update (see CarLocationData)
        public const string EVENT_TR_DATA = "track_data"; //Track data (an array of SegmentData)
        public const string EVENT_CAR_DATA = "car_data"; //Car data (an array of CarData)
        ...
        public const string SV_CAR_C_LIGHTS = "car_c_lights"; //Complex lights, a string id and array of LightData (see LightData) Min 1, Max 3
        ...
        public const string SV_SCAN = "scan"; //Scan for cars (used to start a scan for cars)
        ...
        public static class UtilityMessages { //these are ids for the utility messages (parse them as strings)
            ...
            public const string MSG_CAR_DELOCALIZED = "deloc";//:carID (used to indicate a car is not on the track)
            public const string MSG_CAR_SPEED_UPDATE = "sud";//:carID:speed:trueSpeed (Something internal has changed the speed)
            public const string MSG_CAR_POWERUP = "pup"; //:carID (a car has driven on a FnF powerup)
            public const string MSG_TR_SCAN_UPDATE = "skup"; //:carID:trackValidated (true/false/in-progress)
            public const string MSG_CAR_STATUS_UPDATE = "csu"; //:carID (a cars status has changed, call /cars)
            public const string MSG_LINEUP = "lu"; //:carID:remainingCars (if 0 then lineup is complete)
            public const string MSG_CAR_FLASH_PROGRESS = "cfp"; //:carID:currBytes:totalBytes (both ints, used to indicate the progress of a car flash)
        }
