...
# Alert
This version of the server is incomplete and for reasons i cannot disclose will not be maintained publicly.  
This will be updated to the complete version when possible.  
Sorry for any inconvenience.  

Overdrive Server is a server that can connect to Anki Overdrive cars and control them. It is written in C# and uses an platform selected bluetooth library to connect to the cars over Bluetooth LE. (currently only Windows is fully supported)

Overdrive Server has a web interface that can be used to control the cars, useful for development. 
`http://localhost:7117`
